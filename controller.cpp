...
static void Controller_doorOpened(void* controller, void* context) {
  ...
  Log_P(PSTR(" id: ")); Logln(obj->id);
	...
	Log_exit_P(PSTR("Controller_doorOpened"));
}
...
static void Controller_State_Opening_Free(void* controller,
		Controller_event_enum event) {
  Controller_struct* obj = (Controller_struct*) controller;
  Log_entry_P(PSTR("Controller_State_Opening_Free"));
  Log(PSTR(" id: ")); Logln(obj->id);
	...
	case IN_BETWEEN:
	default:
		Log_error_PP(PSTR("Controller_State_Opening_Free"), PSTR("Illegal event"));
		break;
	}

	Log_exit_P(PSTR("Controller_State_Opening_Free"));
}
...
static void Controller_State_Closing_Blocked(void* controller,
		Controller_event_enum event) {
  Controller_struct* obj = (Controller_struct*) controller;
  Log_entry_P(PSTR("Controller_State_Closing_Blocked"));
  Log_P(PSTR(" id: ")); Logln(obj->id);
	...
	Log_exit_P(PSTR("Controller_State_Closing_Blocked"));
}

static void Controller_State_Closing_Free(void* controller,
    Controller_event_enum event) {
  Controller_struct* obj = (Controller_struct*) controller;
  Log_entry_P(PSTR("Controller_State_Closing_Free"));
  Log_P(PSTR(" id: ")); Logln(obj->id);
  ...
  case IN_BETWEEN:
  default:
    Log_error_PP(PSTR("Controller_State_Closing_Free"), PSTR("Illegal event"));
    break;
  }

  Log_exit_P(PSTR("Controller_State_Closing_Free"));
}
...
static void Controller_autoCloseCB(void* controller, void* context) {
  Controller_struct* obj = (Controller_struct*) controller;
  Log_entry_P(PSTR("Controller_autoCloseCB"));

  Logln_P(PSTR("Pulse low"));
  HSI_writePin(obj->doorRemote, 0);
  
  Log_exit_P(PSTR("Controller_autoCloseCB"));
}
